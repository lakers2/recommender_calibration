import torch
# from torch.autograd import Variable
import torch.nn.functional as F
# from torch import linalg as LA

import time
import numpy as np
import pickle
import argparse
import pandas as pd
import utility
from scipy.sparse import csr_matrix, rand as sprand
from tqdm import tqdm

np.random.seed(0)
torch.manual_seed(0)

class MatrixFactorization(torch.nn.Module):
    def __init__(self, arguments, train_df):
        super(MatrixFactorization, self).__init__()

        self.num_users = arguments['num_user']
        self.num_items = arguments['num_item']
        self.learning_rate = arguments['learning_rate']
        self.epochs = arguments['epoch']
        self.display = arguments['display']
        self.regularization = arguments['reg']
        self.hidden = arguments['hidden']
        self.neg_sampling = arguments['neg']
        self.data = arguments['data']
        self.batch_size = arguments['bs']

        self.train_df = train_df
#         self.group_weight_matrix = group_weight_matrix
#         self.users = users
#         self.ptt2int = ptt2int
#         self.candidate_news = candidate_news

        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.cpu = torch.device("cpu")

        print('******************** MF ********************')

        self.user_factors = torch.nn.Embedding(self.num_users, self.hidden)  # , sparse=True
        self.item_factors = torch.nn.Embedding(self.num_items, self.hidden)  # , sparse=True
        print('P: ', self.user_factors)
        print('Q: ', self.item_factors)

        # initialize small weights
        # self.user_factors.weight.data.uniform_(0, 0.05)
        # self.item_factors.weight.data.uniform_(0, 0.05)

        self.optimizer = torch.optim.Adam(self.parameters(), lr=self.learning_rate, weight_decay=self.regularization)

        print('********************* MF Initialization Done *********************')

        self.to(self.device)

    def forward(self, user, item):
        # Get the dot product per row
        u = self.user_factors(user)
        v = self.item_factors(item)
        x = (u * v).sum(1)

        #         print("U: ", u.shape, u)
        #         print("V: ", v.shape, v)
        #         print("X: ", x.shape, x)
        #         print("=====================")
        #         print(self.user_factors.weight)
        #         print(self.item_factors.weight)
        #         print("===================================================")
        return x

    def train_model(self, itr, weight_matrix, is_weight_change):
        epoch_cost = 0.
        start = time.time()
        self.user_list, self.item_list, self.label_list = utility.negative_sampling(self.num_users, self.num_items,
                                                                                    self.train_df['userId'].values,
                                                                                    self.train_df['itemId'].values,
                                                                                    self.neg_sampling)
        end = time.time()
        print("negative_sampling spends %s" %str(end-start))

#         print('user_list-->  ', self.user_list)
#         print('item_list-->  ', self.item_list)
#         print('label_list-->  ', self.label_list)

        start_time = time.time() * 1000.0
        num_batch = int(len(self.user_list) / float(self.batch_size)) + 1

        num_batch =  60
        self.batch_size = int(len(self.user_list) / float(num_batch)) + 1

        random_idx = np.random.permutation(len(self.user_list))

        for i in tqdm(range(num_batch)):
            #             print(i)
            batch_idx = None
            if i == num_batch - 1:
                batch_idx = random_idx[i * self.batch_size:]
            elif i < num_batch - 1:
                batch_idx = random_idx[(i * self.batch_size):((i + 1) * self.batch_size)]

            tmp_cost = self.train_batch(self.user_list[batch_idx], self.item_list[batch_idx],
                                                              self.label_list[batch_idx], weight_matrix, is_weight_change)

            epoch_cost += tmp_cost

        print("Training //", "Epoch %d //" % itr, " Total cost = {:.5f}".format(epoch_cost),
              "Training time : %d ms" % (time.time() * 1000.0 - start_time))

    def train_batch(self, user_input, item_input, label_input, weight_matrix, is_weight_change):
        users = torch.Tensor(user_input).long().to(self.device)
        items = torch.Tensor(item_input).long().to(self.device)
        labels = torch.Tensor(label_input).float().to(self.device)


        if is_weight_change:
            tmp_weight = []

            for i in range(len(user_input)):

#                 user_stance = self.users[user_input[i]]['ptt']
#                 user_stance_idx = self.ptt2int[user_stance]+2
#
#                 top_news = candidate_news[candidate_pk]
#                 new_stance = int(top_news['source_partisan_score'])+2

                tmp_weight.append(weight_matrix[user_input[i]][item_input[i]])

            tmp_weights = torch.Tensor(tmp_weight).float().to(self.device)

            self.train()
            y_hat = self.forward(users, items)
    #         loss = F.mse_loss(y_hat, labels)
    #         print('y_hat-->', y_hat)
    #         print('labels--> ', labels)
    #         print('tmp_weight--> ', tmp_weights)
#             print('tmp_weight-->',tmp_weight)
#             print('tt--->', tmp_weights*((y_hat-labels)**2))

            loss = torch.sum(tmp_weights*((y_hat-labels)**2))
#             loss = torch.sum(((y_hat-labels)**2))
        else:
            self.train()
            y_hat = self.forward(users, items)
            loss = torch.sum((y_hat-labels)**2)

#             WH = self.reconstruct(H, W)
#             loss = beta_div(WH, V, beta)
#             loss = loss.mul(2).sqrt().item()
#             self.train()
#             y_hat = self.forward(users, items)
#             loss = F.mse_loss(y_hat, labels)*0.5

#         loss = loss.mul(2).sqrt().item()
        # reset gradients
        self.optimizer.zero_grad()
        # backpropagate
        loss.backward()
        # update
        self.optimizer.step()

        self.eval()

        return loss

    def run(self, weight_matrix, is_weight_change):
        best_metric = -1
        best_itr = 0
        for epoch_itr in range(1, self.epochs + 1):
            self.train_model(epoch_itr, weight_matrix, is_weight_change)
            # if epoch_itr % self.display == 0:
            #     cur_metric = self.test_model(epoch_itr)
            #     if cur_metric > best_metric:
            #         best_metric = cur_metric
            #         best_itr = epoch_itr
            #         self.make_records(epoch_itr)
            #     elif epoch_itr - best_itr >= 30:
            #         break

    def make_records(self, itr):  # record all the results' details into files
        P, Q = self.user_factors.weight, self.item_factors.weight
        P = P.detach().cpu().numpy()
        Q = Q.detach().cpu().numpy()
        # with open('./' + self.args.data + '/P_MF_' + str(self.reg) + '.npy', "wb") as f:
        #     np.save(f, P)
        # with open('./' + self.args.data + '/Q_MF_' + str(self.reg) + '.npy', "wb") as f:
        #     np.save(f, Q)
        return P, Q

# if __name__ == '__main__':
#     args = {
#         'epoch': 40,
#         'display': 1,
#         'learning_rate': 0.001,
#         'reg': 0.001,
#         'hidden': 40,
#         'neg': 2,
#         'bs': 1024,
#         'data': 'ML1M'
#     }
#
#     with open('./ML1M/info.pkl', 'rb') as f:
#         info = pickle.load(f)
#         args['num_user'] = info['num_user']
#         args['num_item'] = info['num_item']
#     print(info)
#
#     train_like = list(np.load('./ML1M/user_train_like.npy', allow_pickle=True))
#     test_like = list(np.load('./ML1M/user_vali_like.npy', allow_pickle=True))
#     train_df = pd.read_csv('./ML1M/train_df.csv')
#
#     model = MatrixFactorization(args, train_df)
#     model.run()