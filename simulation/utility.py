...
def negative_sampling(num_user, num_item, pos_user_array, pos_item_array, neg_rate):
    ...
    user_neg = np.random.choice(np.arange(num_user), size=(int)(neg_rate * pos_user_array.shape[0]), replace=True)
    pos = pos_item_array
    neg = np.random.choice(np.arange(num_item), size=(int)(neg_rate * pos_user_array.shape[0]), replace=True)
